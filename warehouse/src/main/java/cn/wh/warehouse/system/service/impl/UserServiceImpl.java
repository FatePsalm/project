...
import cn.wh.warehouse.common.web.SpringListUtil;
import cn.wh.warehouse.system.dao.RoleDao;
import cn.wh.warehouse.system.dao.UserDao;
import cn.wh.warehouse.system.dao.UserRoleDao;
...
//		String pwd = HashUtil.hashMD5Base64(salt, user.getPassword());
	...
	public void updateUser(User user,String roleIdList) {
		...
		String saltStr = UUID.randomUUID().toString();   //生成盐�??
	//	ByteSource salt = ByteSource.Util.bytes(saltStr);
		//String pwd = new SimpleHash("MD5",user.getPassword(),salt).toString();
		//user.setPassword(pwd);
		user.setSalt(saltStr);
	...
	public void changeState(Integer userId, Integer valid) {
		if(userId==null){
			throw new RuntimeException("修改用户状�?�，用户id不能为空�?");
		}
		if(valid==null){
			throw new RuntimeException("修改用户状�?�，valid值不能为空！");
		}
		int i = userDao.changeState(userId,valid);
		if(i!=1){
			throw new RuntimeException("切换用户启用禁用状�?�失败！");
		}
	}
